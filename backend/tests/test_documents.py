...
async def test_sync_call_records_a_document(client, ocr_token, session):
    r = await client.post(
        SYNC, files={"file": ("factura.png", _png(), "image/png")}, headers=_auth(ocr_token)
    )
...
async def test_classify_call_records_a_document(client, ocr_token, session):
    r = await client.post(
        CLASSIFY, files={"file": ("cv.png", _png(), "image/png")}, headers=_auth(ocr_token)
    )
    assert r.status_code == 200, r.text
...
async def test_async_job_document_tracks_the_job(client, ocr_token, session):
    r = await client.post(
        JOBS, files={"file": ("doc.png", _png(), "image/png")}, headers=_auth(ocr_token)
    )
...
async def test_panel_lists_and_filters(admin_client, ocr_token):
    await admin_client.post(
        SYNC, files={"file": ("a.png", _png(), "image/png")}, headers=_auth(ocr_token)
    )
    await admin_client.post(
        CLASSIFY, files={"file": ("b.png", _png(), "image/png")}, headers=_auth(ocr_token)
    )
