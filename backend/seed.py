...
async def seed(db: AsyncSession) -> None:
    ...
    stale = (await db.execute(select(Permission).where(Permission.name.notin_(PERMISSIONS)))).scalars().all()
    if stale:
    ...
    for role_name, perm_names in ROLES.items():
        result = await db.execute(select(Role).where(Role.name == role_name).options(selectinload(Role.permissions)))
    ...
    result = await db.execute(select(User).where(User.email == settings.admin_email).options(selectinload(User.roles)))
