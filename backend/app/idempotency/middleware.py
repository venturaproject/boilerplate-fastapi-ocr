    ...
    async with AsyncSessionLocal() as session, session.begin():
        inserted_id = (
            await session.execute(
                ...
                .values(
                    key=key, principal=principal, method=method, path=path,
        ...
        row = (
            await session.execute(
                select(IdempotencyKey).where(
                    IdempotencyKey.key == key, IdempotencyKey.principal == principal
                )
            )
        ).scalar_one()
        return (
        ...
        if status == 0 or status >= 500:
            await session.execute(
                delete(IdempotencyKey).where(
                    IdempotencyKey.id == record_id, IdempotencyKey.response_status == 0
                )
            )
            return
        try:
