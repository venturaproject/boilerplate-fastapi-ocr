...
async def stats(db: AsyncSession, *, api_client_id: uuid.UUID | None = None) -> DocumentStats:
    ...
    total = (await db.execute(select(func.count()).select_from(Document).where(*base_filters))).scalar_one()

    cutoff_24h = datetime.now(tz=UTC) - timedelta(hours=24)
    last_24h = (
        await db.execute(
            select(func.count()).select_from(Document).where(*base_filters, Document.created_at >= cutoff_24h)
        )
    ).scalar_one()
    ...
    by_mode = {
        m: int(n)
        for m, n in (
            await db.execute(select(Document.mode, func.count()).where(*base_filters).group_by(Document.mode))
        ).all()
    }
    by_status = {
        s: int(n)
        for s, n in (
            await db.execute(select(Document.status, func.count()).where(*base_filters).group_by(Document.status))
        ).all()
    }
    by_doc_type = {
        dt: int(n)
    ...
    avg_ms = (await db.execute(select(func.avg(Document.processing_ms)).where(*done_filter))).scalar_one_or_none()
    p95_ms = (
        await db.execute(
            select(func.percentile_cont(0.95).within_group(Document.processing_ms.asc())).where(*done_filter)
        )
    ).scalar_one_or_none()
...
async def purge_expired_documents(db: AsyncSession, *, retention_days: int, limit: int = 500) -> int:
    """Delete document rows older than the retention window (0 disables)."""
    if retention_days <= 0:
        return 0
    cutoff = datetime.now(tz=UTC) - timedelta(days=retention_days)
    result = await db.execute(select(Document.id).where(Document.created_at < cutoff).limit(limit))
