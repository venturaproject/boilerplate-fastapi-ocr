...
async def ocr_scan(
    ...
    """Synchronous OCR playground. `?format=json|text|hocr|alto|pdf`."""
    out = await run_sync_ocr(file, lang, fmt=format, created_by_user_id=user.id)
...
async def ocr_get_job(job_id: uuid.UUID, format: str = "json", db: AsyncSession = Depends(get_db)) -> OcrJobOut:
    job = await ocr_repo.get_job(db, job_id)
    if not job:
        raise NotFoundException("Job no encontrado")
    if format != "json":
        from app.routers.ext_ocr import _job_formatted

        return _job_formatted(job, format)  # type: ignore[return-value]
    return OcrJobOut.model_validate(job)


@router.post("/jobs/{job_id}/redeliver", dependencies=[require_permission("ocr.jobs.view"), _ocr_rl])
async def ocr_redeliver(job_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> OcrJobOut:
    job = await ocr_repo.get_job(db, job_id)
    if not job:
        raise NotFoundException("Job no encontrado")
