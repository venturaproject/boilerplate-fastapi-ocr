...
async def list_permissions(
    ...
    page = max(1, page)
    perms, total = await perm_repo.list_permissions(db, search=search, group=group, page=page, per_page=per_page)
...
async def get_permission(perm_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> PermissionOut:
