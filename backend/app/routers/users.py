...
async def create_user(
    ...
    request: Request,
    actor: User = require_permission("users.create"),
    db: AsyncSession = Depends(get_db),
) -> UserOut:
    ...
    await audit_log(request, actor, "user.created", user.email, roles=body.role_ids)
...
async def update_user(
    ...
    request: Request,
    actor: User = require_permission("users.edit"),
    db: AsyncSession = Depends(get_db),
) -> UserOut:
    user = await user_repo.get_user_by_id(db, user_id)
    if not user:
    ...
    await audit_log(
        ...
        "user.updated",
        updated.email,
        changed=sorted(k.replace("password_hash", "password") for k in update_kwargs),
    )
...
async def delete_user(
    user_id: uuid.UUID,
    request: Request,
    actor: User = require_permission("users.delete"),
    db: AsyncSession = Depends(get_db),
) -> None:
    user = await user_repo.get_user_by_id(db, user_id)
    if not user:
    ...
    await audit_log(request, actor, "user.deleted", email)
