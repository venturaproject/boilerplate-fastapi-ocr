...
export interface DocExtractionField {
  value: string
  raw: string | null
  confidence: number
}
...
export interface DocExtraction {
  doc_type: string | null
  fields: Record<string, DocExtractionField>
}

export interface OcrResult {
  engine: string
  lang: string
  lang_detected?: boolean
  page_count: number
  pages: OcrPage[]
  text: string
...
export const ocrApi = {
  ...
  /** Re-run the sync OCR asking for a specific output format; returns a Blob. */
  async scanAs(file: File, format: 'text' | 'hocr' | 'alto' | 'pdf', lang?: string): Promise<Blob> {
    const { data } = await axios.post(endpoints.ocr.scan, buildForm(file, lang), {
      headers: { 'Content-Type': 'multipart/form-data' },
      params: { format },
      responseType: 'blob',
  ...
  async jobAs(id: string, format: 'text' | 'hocr' | 'alto'): Promise<Blob> {
  ...
  async redeliver(id: string): Promise<void> {
    await axios.post(`${endpoints.ocr.jobs}/${id}/redeliver`)
  },
