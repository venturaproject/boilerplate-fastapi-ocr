...
import type { DocumentMode, DocumentRow, DocumentStatus } from '@/services/documents-api'
import { DOC_TYPE_LABEL } from '@/pages/ocr/index'

export const MODE_LABEL: Record<DocumentMode, string> = {
  sync: 'Síncrono',
  async: 'Asíncrono',
...
export function buildDocumentsColumns(t: (k: string) => string) {
  return [
    ...
    col.accessor('mode', {
      id: 'mode',
      header: () => t('col_mode') || 'Modo',
      cell: (info) => <Badge variant="secondary">{MODE_LABEL[info.getValue()]}</Badge>,
    }),
    col.accessor('status', {
      id: 'status',
...
export const documentColumnLabels = (t: (k: string) => string): Record<string, string> => ({
  original_filename: t('col_file') || 'Archivo',
  mode: t('col_mode') || 'Modo',
